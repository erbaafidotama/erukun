...
// 	// select * from User
// 	if err := config.DB.Find(&wargas).Error; err != nil {
// 		// return error
// 		c.JSON(404, gin.H{
// 			"status":  "error",
...
// 	if err := config.DB.Where("warga_uuid = ?", wargaUuid).First(&dataWarga).Error; err != nil {
// 		c.JSON(404, gin.H{
// 			"status":  "error",
// 			"message": "record not found",
// 		})
...
// 	config.DB.Model(&dataWarga).Where("warga_uuid = ?", wargaUuid).Updates(models.WargaMaster{
...
// 	config.DB.Where("warga_uuid = ?", wargaUuid).Delete(&dataWarga)
